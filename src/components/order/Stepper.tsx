...
const Step: React.FC<StepProps> = ({
  ...
  return <div className="items-center flex gap-0.5 flex-1 shrink basis-[0%] pr-0.5 px-0">
      <div className="self-stretch flex flex-col items-center text-xs text-[#32373E] font-normal whitespace-nowrap text-center tracking-[0.06px] w-5 my-auto">
        {isCompleted ? <div className="w-5 h-5 rounded-full bg-[#0072EF] flex items-center justify-center">
            <Check className="w-3.5 h-3.5 text-white" />
          </div> : isOrder && isActive ? <div className="w-5 h-5 rounded-full bg-[#0072EF] flex items-center justify-center">
            <ShoppingBasket className="w-3 h-3 text-white" />
          </div> : <div className={`w-5 h-5 rounded-full ${isActive ? 'bg-[#8E9196]' : 'bg-[#E6EBF0]'} flex items-center justify-center`}>
            {isActive && <Truck className="w-3.5 h-3.5 text-white" strokeWidth={3} />}
          </div>}
        <div className={`text-[#32373E] mt-1 text-center ${isMobile ? 'text-[10px]' : ''}`}>{isMobile && label.length > 6 ? label.substring(0, 6) : label}</div>
      </div>
      {!isLast && <div className={`self-stretch ${isMobile ? 'min-w-8' : 'min-w-20'} flex-1 shrink basis-[0%] my-auto relative`}>
